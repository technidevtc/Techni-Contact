<?php
	...
	//Looking for the ID's of the Family ND
	$query_get_infos2		="SELECT
									id
								FROM
									families
								WHERE
	...
	$res_get_infos2 		= $db->query($query_get_infos2, __FILE__, __LINE__);
	...
	while($data_get_infos2 	= $db->fetchAssoc($res_get_infos2)){
		
		//Looking for the Third level for every returned row
		$query_get_infos3		="SELECT
									id
								FROM
									families
								WHERE
									idparent=".$data_get_infos2['id']."
	...
	}//End while fetching the second level
