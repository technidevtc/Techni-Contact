<?php 
	...
	if(strpos($content_get_user_page_permissions['content'],'#18#')===FALSE){
	...
	}else if(empty($email_id) || empty($motif)){
		...
		echo('<div id="base_emails_final_results_container">');
			...
			echo('</div>');
			echo('<div>');
				echo('<font color="green">Adresse d&eacute;sabonner avec succ&egrave;s !</font>');
			echo('</div>');
			
		echo('</div>');
		
		//Log the actions (Global Count, Count Add, Count Edit).
		$base_emails_log	= "Base Email: BlackList One \n ";
		$base_emails_log	.= "Email ID: ".$email_id." \n ";
		$base_emails_log	.= "Motif ID: ".$motif." \n ";
