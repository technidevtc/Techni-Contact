<?php 
	...
	if(strpos($content_get_user_page_permissions['content'],$page_id)===FALSE){
		...
		$wb_segment_name		="segment_create";
		...
		if(!empty($segment_used_table) && !empty($segment_typology) && !empty($segment_name) && !empty($content)){
			try{
				...
				if(!empty($groupes)){
					...
					$query_segment_insert	="	INSERT INTO marketing_segment  
													(id, id_table,
													name, type,
													condition_from, condition_group,
													date_creation, id_user)
													
												VALUES
													(NULL, ".$segment_used_table.",
													'".$segment_name."', '".$segment_typology."', 
													'".$segment_condition_from."', '".$segment_condition_group."', 
													NOW(), ".$_SESSION['marketing_user_id'].")";
													
					$res_segment_insert 	= $db->query($query_segment_insert, __FILE__, __LINE__);
					
					$segment_id	= mysql_insert_id();
					//$segment_id = 1;
					if(empty($segment_id)){
						throw new Exception('Erreur dans la cr&eacute;ation de segment !');
					}
...
echo('<br /><br />');
					...
					echo('<div id="segmet_final_results_container">');
						...
						echo('</div>');
						echo('<div>');
							echo('<font color="green">Segment cr&eacute;er avec succ&egrave;s !</font>');
						echo('</div>');
						
						echo('<div id="segmet_final_results_container_js" style="display:none;">');
							echo('redirect_page_after_segment_create();');
						echo('</div>');
					echo('</div>');
					echo('<br /><br />');
					...
					$query_insert_history	="INSERT INTO  marketing_users_history(id, action, 
																		id_user, action_date)
									VALUES(NULL, 'Create Segment ID: ".$segment_id."',
