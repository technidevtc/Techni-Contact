//Load the segment fields
function segment_edit_load_the_fields(){
...
function segment_edit_validation(){
	...
	var segment_id					= document.getElementById('hidden_segment_id').value;
	...
	if(segment_id==''){
		global_errors_validation	= '<div class="segment_errors_head">- L\'<b>ID</b> du segment est introuvable !</div>';
		etat 	= 0;
	}
	...
	global_informations_send	+= '<content id="segment_edit">';
	...
	if(etat==1){
		...
		OAjax.open('POST','/fr/marketing/segments-edit-confirm.php',true);
		...
		OAjax.send('segment_id='+segment_id+'&segment_used_table='+segment_used_table+'&segment_typology='+segment_typology+'&segment_name='+segment_name+'&content='+global_informations_send);
...
function redirect_page_after_segment_edit(){
