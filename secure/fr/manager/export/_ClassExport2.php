	...
	*/
	...
	function Create($partner_id, $parent_id = 0)
	{
		...
		if ($this->handle->numrows($result, __FILE__, __LINE__) == 1)
		{
			...
			$this->compulsory_fields = mb_unserialize($this->compulsory_fields);
			$this->facultative_fields = mb_unserialize($this->facultative_fields);
			...
			if (is_string($ids) && strtolower($ids) == "all")
			{
				...
				if (($nb_pdt = $this->handle->numrows($result, __FILE__, __LINE__)) > 0)
				{
					while ($rec = & $this->handle->fetch($result))
						$this->products[$rec[0]] = array(mb_unserialize($rec[1]), mb_unserialize($rec[2]));
					$this->nb_pdt = $nb_pdt;
				}
				else
				{
	...
	function AddProduct($idProduct, $idFamily)
	{
		...
		$result = & $this->handle->query("select p.id, p.idAdvertiser, p.idTC, pfr.name, pfr.fastdesc, pfr.ref_name, pfr.alias, pfr.keywords, pfr.descc, pfr.descd, pfr.delai_livraison as delivery_time, p.refSupplier, p.price, p.price2, p.unite, p.idTVA " .
			"from products p, products_fr pfr, products_families pf " .
			"where p.id = pfr.id and p.id = " . $idProduct . " and p.id = pf.idProduct and pf.idFamily = " . $idFamily, __FILE__, __LINE__, false);
		...
		if ($this->handle->numrows($result, __FILE__, __LINE__) == 1)
		{
			...
			$content_headers = mb_unserialize($ref_col_content);
