...
/*================================================================/
 ...
 Fichier : /secure/manager/import/suppliers.php
 Description : Mise à jour des fournisseurs par import
...
<div class="blocka"><a href="javascript: newimport.Show()">Mettre à jour un fournisseur</a></div>
<div class="legend-group">
	Légende :
	<span class="legend-label not-valid">non importable</span>
	<span class="legend-label valid">en attente</span>
	<span class="legend-label finalized">importé</span>
	<span class="legend-label cancelled">import annulé</span>
</div>
<div class="zero"></div>
<div id="ImportWindow">
	<form id="ImportWindowForm" action="make_import.php?typeImport=<?php echo __IMPT_UPDT_SUPPLIER__ ?>" method="post" enctype="multipart/form-data">
...
it.setHeaders(["ID import", "Nom fournisseur", "Date de la mise à jour", "Nb de produits mis à jour", "Nb d'erreurs", function(val, rowh) { rowh.className = val; } ]);
/*it.setHeaders([
...
$result = & $handle->query("select i.id, a.nom1, i.nbp_final , i.timestamp, i.filename, i.status from imports i, advertisers a where i.idAdvertiser = a.id".
        " and type = ". __IMPT_UPDT_SUPPLIER__ , __FILE__, __LINE__, false);
$nbimp = $handle->numrows($result, __FILE__, __LINE__);
for ($i = 0; $i < $nbimp; $i++)
{
	$rec = & $handle->fetchAssoc($result);

        $imp = & new Import($handle, $rec['id']);
	...
	switch ($rec['status'])
	{
        ...
        $nbp_final = $rec['nbp_final'] == 0 ? 'N/C' : $rec['nbp_final'];
	print '	["' . $rec['id'] . '", "' . $rec['nom1'] . '", "' . date("Y/m/d H:i", $rec['timestamp']) . '", "' . $nbp_final . '", "' . ($nbRefWithoutProduct+$nbp_final-$nbProductByFileRef) . '", "' . $statusClass . '"]' . ($i < ($nbimp-1) ? "," : "")  . "\n";
}
/*
for ($i = 0; $i < 10; $i++)
{
...
it.setEditTools( {
	"edit" : {"element" : "div", "attributes" : { "onclick" : function() { document.location.href = "supplier.php?id=" + this.parentNode.parentNode.cc[0].textvalue; } } }
} );
it.Refresh();
//it.PurgeAll();
