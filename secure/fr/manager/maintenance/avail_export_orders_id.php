...
while ($cmd = $db->fetchAssoc($res)) {
  ...
  foreach ($cmd_lines as $cmd_line) {
    $db->query("INSERT INTO `orders_lines_tmp` (`UserID`,`ProductID`,`OrderID`) VALUES ('".$cmd["idClient"]."','".$cmd_line["idProduct"]."','O".$cmd["id"]."')",__FILE__,__LINE__);
  }
}

// leads
$res = $db->query("
  SELECT cus.id as user_id, lead.idProduct AS product_id, lead.id as lead_id
  FROM contacts lead
  ...
  WHERE lead.timestamp > ".$starting_date."  ",__FILE__,__LINE__);
