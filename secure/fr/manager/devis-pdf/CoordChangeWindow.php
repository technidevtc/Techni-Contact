...
function saveShipAddress()
{
	...
	else
	{
		...
		fieldlist += '&cp_l=' + escape(document.coord.cp_l.value);
		fieldlist += '&ville_l=' + escape(document.coord.ville_l.value);
		fieldlist += '&pays_l=' + escape(document.coord.pays_l.value);
		fieldlist += '&infos_sup_l=' + escape(document.coord.infos_sup_l.value);
	}
	AlterEstimate('&alterShipAddress=1' + fieldlist);
}
		...
		<div class="window_bg">
			<form id="coord" name="coord" method="post" action="commande_coord.html">
				...
				<div id="coordlivraisonshown">
					...
					<div class="coord_norm"><div class="intitule">Complément (ZI, BP, etc) :</div><input type="text" name="complement_l" size="20" maxlength="20" value="" /></div>
					<div class="coordfloat" style="margin-right: 50px"><div class="intitule">CP :</div><input type="text" name="cp_l" size="5" maxlength="6" value="" onBlur="this.value = parseInt(trim(this.value))" /></div>
