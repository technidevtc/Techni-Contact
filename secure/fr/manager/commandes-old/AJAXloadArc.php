...
<script type="text/javascript">
 //CQDB = Charge Question Dialog Box
  ...
  $("form[name='loadArc']").submit();
  });
</script>
<form name="loadArc" method="post" action="" enctype="multipart/form-data">
<input type="hidden" name="load-arc" value="1"/>
<input type="hidden" name="supplier" value="<?php echo $_GET['idSupplier'] ?>"/>
<input type="hidden" name="cmdId" value="<?php echo $cmd->id ?>"/>
S&eacute;lectionnez l'ARC &agrave; lier &agrave; la commande <?php echo $_GET['idSupplier'] ?>-<?php echo $cmd->id ?> <br/>
 <br/>
<input type="file" name="arcFile"  accept="application/pdf" /><br/>
 <br/>
 <input type="button" value="Annuler"/> &nbsp; &nbsp; <input type="button" value="Lier ARC"/>
</form>
 <?php
        }
  }
