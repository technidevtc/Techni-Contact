...
function & loadProduct(& $handle, $id, $idFamily)
{
	...
	" and pf.idFamily = " . $idFamily . " and p.id = pf.idProduct and p.id = pg.id and a.id = pg.idAdvertiser and a.actif = 1 and a.parent = 61049";
...
if(!isset($_GET['family_ref_name']))
{
	...
	if($pdt['price'] == 'ref')
	{
	    ...
	    if ($price2_present)
		{
			$result = & $handle->query("select id, label, content, refSupplier, price, idTVA, unite from references_content where idProduct = " . $pdt['id'] . " order by classement", __FILE__, __LINE__, false);
			...
			<div class="prixProduit">
				<form method="post" action="">
					...
					<table id="prixProduit" cellspacing="0" cellpadding="0"><tr>
						...
						<td><img src="../ressources/buy.gif" alt="Acheter" onClick="AddProduct(<?php echo $pdt['id'] ?>, '', document.getElementById('ProductQuantity').value)" style="cursor: pointer" /></td>
					</tr></table>
				</form>
			</div>
<?php
...
<?php
			...
			while ($pdt = & $handle->fetchAssoc($result))
			{
				...
				{
					if ($pdt['price2'] == '0' || !preg_match('/^[0-9]+((\.|,)[0-9]+){0,1}$/', $pdt['price2'])) continue;
				}
				elseif ($pdt['price'] == 'ref')
				{
					...
					$tab_ref_cols = mb_unserialize($data[0]);
					if ($tab_ref_cols[2] != 'Référence Fournisseur') continue;
				}
				else continue;
