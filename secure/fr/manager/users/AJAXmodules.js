...
HN.Mods.MISM = function() {
	...
	this.GetSelectedItems = function () {
		...
		for (var catID in SIbC) {
			ssi += (k++ > 0 ? "|" : "") + catID;
			for (var itemID in SIbC[catID])
				ssi += "," + itemID;
		}
		return ssi;
	};
	
	this.GetSelectedItemsJSON = function () {
		var k = 0;
		var osi = [];
		for (var catID in SIbC)
			for (var itemID in SIbC[catID])
				osi.push({ id: itemID, catID: catID });
		return osi;
	};
							...
							PdtHdl.GetProductsByFamID([item_cat2.cat3[this.index].id], function (items) {
								...
								*/
								...
								for (var id in items) {
									...
									var item = items[id];
									if (SIbC[item_cat3.id] && typeof SIbC[item_cat3.id][item.id] != "undefined")
										SIbC[item_cat3.id][item.id] = true;
									
									var	item_block = document.createElement("a");
									item_block.className = "item-block";
									if (SIbC[item_cat3.id] && SIbC[item_cat3.id][item.id]) item_block.className += " selected";
									item_block.href = "id:" + item.id;
									...
									item_image.src = "../AJAXressources/modules/products-images.php?id=" + item.id;
									item_image.alt = item.id + " image";
									...
									item_desc.innerHTML = item.fastdesc;
