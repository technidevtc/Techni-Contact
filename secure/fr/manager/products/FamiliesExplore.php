...
if(!isset($_GET['family_ref_name']))
{
	...
	$isFromSupplier = $pdt['parent'] == 61049 ? true : false;
	...
	{
		if ($pdt['price2'] != '0' && preg_match('/^[0-9]+((\.|,)[0-9]+){0,1}$/', $pdt['price2'])) $price2_present = true;
		else $price2_present = false;
	}
	else $price2_present = false;
			...
			<div class="prixProduit">
				<form method="post" action="">
					...
					<table id="prixProduit" cellspacing="0" cellpadding="0"><tr>
						...
						<td class="quantiteLabel">Quantité :</td><td class="quantite"><input type="text" id="ProductQuantity" size="6" maxlength="5" value="1"></td>
						<td><img src="<?php echo URL ?>ressources/buy.gif" alt="Acheter" onClick="AddProduct(<?php echo $pdt['id'] ?>, '', document.getElementById('ProductQuantity').value)" style="cursor: pointer" /></td>
					</tr></table>
				</form>
			</div>
<?php
...
<?php	for($i = 3; $i < count($tab_ref_cols)-5; ++$i)
		{
			$ref_content .= "							<th>" . to_entities($tab_ref_cols[$i]) . "</th>\n";
		}
?>							<th>Unit&eacute;</th>
							<th>Prix HT</th>
							<th>Qt&eacute;</th>
...
<?php		for($j = 0; $j < count($content); ++$j)
			{
				...
				$ref_content .= "							<td>" . to_entities($content[$j]) . "</td>\n";
			}
?>							<td><?php echo to_entities($ref['unite']) ?></td>
							<td class="ref-prix"><?php echo to_entities(sprintf('%.02f',$ref['price'])) ?>€</td>
							<td class="ref-qte"><input type="text" id="ProductQuantity<?php echo $ref['id'] ?>" value="1"></td>
							<td class="ref-buy"><img src="<?php echo URL ?>ressources/buy.gif" alt="Acheter" onclick="AddProduct(<?php echo $pdt['id'] ?>, <?php echo $ref['id'] ?>, document.getElementById('ProductQuantity<?php echo $ref['id'] ?>').value)" /></td>
						</tr>
<?php	}
?>						</tbody>
					</table>
...
else
{
	...
	if ($family['child'] == null)
	{
		...
		$result = & $handle->query("select count(pfam.idProduct) from products_families pfam, products p, advertisers a where pfam.idProduct = p.id and p.idAdvertiser = a.id and a.parent = 61049 and idFamily = " .$cur_family['id'] . " and a.actif = 1", __FILE__, __LINE__);
...
<?php
			...
			print "	<div id=\"liste\">\n";
			$n = 0;
			$result = & $handle->query("select p.id, pf.ref_name, pf.name, pf.fastdesc, p.price, p.price2 from products p, products_fr pf, products_families pfam, advertisers a where p.id = pf.id and p.id = pfam.idProduct and p.idAdvertiser = a.id and a.actif = 1 and pfam.idFamily = " . $cur_family['id'] . " order by pf.ref_name limit " . (($page-1)*NB) . "," . NB, __FILE__, __LINE__);
...
<?php
		...
		foreach ($top_family['child'] as $s_fam)
		{
			...
			{
				//$toprint .= '		<a href="familles/' . $ss_fam['ref_name'] . '">' . to_entities($ss_fam['name']) . " (" . $ss_fam['nbpdt'] . ")</a>\n";
				$toprint .= '		<a href="familles/' . $ss_fam['ref_name'] . '">' . to_entities($ss_fam['name']) . "</a>\n";
				//$nbpdt   += $ss_fam['nbpdt'];
			}
			//print '	<div class="elt_float" id="folder'. $s_fam['id'] . '_s"><strong>' . to_entities($s_fam['name']) . " (" . $nbpdt . ")</strong>\n";
			print '	<div class="elt_float" id="folder'. $s_fam['id'] . '_s"><strong>' . to_entities($s_fam['name']) . "</strong>\n";
			print $toprint;
			print "	</div>\n";
