...
if ($SuppliersSearchText != '')
{
	...
	if ($sbb) $regexp .= '^';
	$regexp .= $sst;
	if ($result = $handle->query("select id, nom1 from advertisers where nom1 regexp " . ($scs ? 'binary ' : '') . "\"" . $handle->escape($regexp) . "\" and parent = " . __ID_TECHNI_CONTACT__ . " AND deleted != 1 order by nom1", __FILE__, __LINE__))
	{
