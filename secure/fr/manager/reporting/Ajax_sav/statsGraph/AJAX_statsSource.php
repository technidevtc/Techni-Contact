...
if (isset($_GET['start']) AND isset($_GET['end'])) {
	...
	if($typeG == 'all'){
		$sqlAdd	= "  ";
				...
				if($typeG == "Autre") 	 $sqlAdd = " AND order_type IN('3','4','5','9') ";
				if($typeG == "Devis") 	 $sqlAdd = " AND order_type IN('1','2') ";
				if($typeG == "Internet") $sqlAdd = " AND order_type='0' ";
	}
	...
	while($results =  mysql_fetch_assoc($reqDay)){
		...
		if(empty($dataCount->total)){
			$value = 0;
		}else $value = $dataCount->total;
		
		$data[$key]['label'] = $results['selected_date'];
