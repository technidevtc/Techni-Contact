...
<table  class="item-list">
	...
	<?php
		...
		$sql_relance_comm  = "SELECT DISTINCT(csv.assigned_operator) ,bu.name
		...
		while($data_relance_comm = mysql_fetch_object($req_relance_comm)){
			...
			$rows_total_ff 		+= $total_sortant_final;
			...
			$rows_not_called_ff 		+= $rows_not_called;
			...
			$rows_call_ok_ff 		+= $rows_call_ok;
			
			$total_joindre  = $rows_not_called + $rows_absent;
			$total_joindre_ff 		+= $total_joindre;
	...
	?>
	  <tr>
		<td><b>Total : </b> </td>	
		<td><b><?= $rows_total_ff ?></b> </td>
		<td><b><?= $total_joindre_ff ?></b> </td>
		<td><b><?= $rows_not_called_ff ?></b> </td>
		<td><b><?= $rows_absent_ff ?></b> </td>
		<td><b>-</b> </td>
	  </tr>
	<?php }else{ ?>
		<tr>
