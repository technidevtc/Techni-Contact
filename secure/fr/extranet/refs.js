...
function loadRefTable()
{
	...
	for(i = 0; i < refRows.length; ++i)
	{
		...
		for(j = 0; j < refRows[i].length; ++j)
		{
			...
			else if (j == refRows[i].length-3)
			{
				out += '	<td ' + (gray ? 'class="gray"' : 'class="reftva"') + ' onContextMenu="showContextMenu(event, ' + j + ', ' + i + '); return false">' + "\n";
				out += '		<select name="idTVA" onBlur="saveCellContent(this.value, ' + j + ', ' + i + ")\">\n";
				out += displayOptionsTVA(getValidTVA(refRows[i][j], idTVAdft), 'rate');
				out += "		</select>\n";
				out += "	</td>\n";
				saveCellContent('' + getValidTVA(refRows[i][j], idTVAdft), j, i);
				//alert(refRows[i][j]);
			}
			else
			{
				...
				out += '<input ' + (j == refRows[i].length-1 || j == refRows[i].length-3 ? 'class="Right" ' : '') + 'type="text" size="3" ';
	...
	for (k = 0; k < tds.length; k++)
	{
		...
		else if (tds[k].className == 'reftva')
		{
			tds[k].onmouseover = function () { this.style.backgroundColor = '#316AC5'; }
			tds[k].onmouseout = function () { this.style.backgroundColor = '#FFFFFF'; }
		}
		else if (tds[k].className != 'gray')
		{
			...
			//i = tds[k].getElementsByTagName('input');
			//i[0].onmouseover = function () { this.style.backgroundColor = '#316AC5'; this.style.color = '#FFFFFF'; }
			//i[0].onmouseout = function () { this.style.backgroundColor = '#FFFFFF'; this.style.color = '#000000'; }
		}
	}
...
function buildContextMenu(x, y)
{
	...
	if(new String(x).match(/^[0-9]+$/) && new String(y).match(/^(\-1|[0-9])+$/) && x < refCols.length && y < refRows.length)
	{
		...
		// Menu coller si donnees a coller si colonne utilisateur
		if(colNumberToPaste > 2 && colNumberToPaste < refCols.length - 4 && x < refCols.length - 4)
		{
			data = 1;
...
function pasteRefColumn(pos)
{
	...
	if(colNumberToPaste > 1 && colNumberToPaste < refCols.length - 4 && new String(pos).match(/^[0-9]+$/) && pos != refCols.length)
	{
