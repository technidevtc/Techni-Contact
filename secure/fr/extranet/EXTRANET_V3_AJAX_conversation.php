...
switch ($action) {
  case 'get':
	...
	//echo $context.' ** '.$_SESSION['extranet_user_id'].' ** '.$ordre;
    if(!empty ($context))
      $data = Messenger::getConversation($context, $_SESSION['extranet_user_id'], $ordre);
  ...
  case 'add':
    ...
    if (!empty($ordre)) { // send message about order to TC
      ...
      $m->id_sender = $_SESSION['extranet_user_id'];
