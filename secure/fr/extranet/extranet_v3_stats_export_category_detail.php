<?php
	...
	$stats_h_month			= mysql_escape_string($_POST['stats_h_month_final']);
	...
	if(!empty($_SESSION['extranet_user_id'])){
		...
		if(!empty($stats_h_data) && !empty($stats_h_values)){
			...
			$workbook->send("Statistiques detail category ".date('d/m/Y h:i:s').".xls");

			$worksheet = $workbook->addWorksheet('Statistiques detail category');
			
			
			//fetch received informations
			$stats_h_month			= stripslashes(str_replace("\r\n","", $stats_h_month));
			$stats_h_month_temp		= explode('#',$stats_h_month);
			
			$worksheet->write($local_loop_excel, 0, ucfirst(utf8_decode($stats_h_month_temp[0])).' '.$stats_h_month_temp[1]);
			...
			//Headers
			if(!empty($stats_h_month_temp[1])){
				//$worksheet->write($local_loop_excel, 0, ucfirst(utf8_decode($stats_h_month_temp[1])));				
				$worksheet->write($local_loop_excel, 1, 'Nombre de vues');
				$local_loop_excel++;
			}else{
				//$worksheet->write($local_loop_excel, 0, "Période");
				$worksheet->write($local_loop_excel, 1, 'Nombre de vues');
				$local_loop_excel++;
			}
			
			
			//Make a spacement of two rows
			$local_loop_excel++;
