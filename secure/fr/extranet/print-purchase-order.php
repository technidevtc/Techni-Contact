...
<head>
  ...
  <style type="text/css">
    ...
    .bill_total { width: 50%; border: black solid 1px; border-collapse: collapse; float: right }
    .bill_total tbody tr td { border: black solid 1px; padding: 0px 10px 0px 10px; text-align: right }
    .bill_delivery_info { width: 25%; border: black solid 1px; border-collapse: collapse; float: left }
        ...
        <tbody>
         <?php foreach ($so['order']['lines'] as $line) : ?>
          <tr>
            ...
            <td style="text-align: center"><?php echo sprintf("%.02f", $line['total_a_ht']) ?> €</td>
          </tr>
         <?php if ($line['et_ht'] > 0) : ?>
          <tr>
            <td></td>
