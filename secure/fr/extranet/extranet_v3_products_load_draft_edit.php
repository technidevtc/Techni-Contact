		...
		if($total_count_results!=0){
			...
			echo('<div id="page_list_selector_container" style="padding: 25px 0 15px 10px; width:40%; float:left;">');
				...
				echo('<div id="page_list_selector_select">');
					...
					echo('<select id="f_select_p-edit" onchange="products_draft_get_more_now(\'edit\')">');
						echo('<option value="5"');
						if(strcmp($f_pp,5)==0){
							echo(' selected="true" ');
						}
			...
			echo('<div class="pagination products-top-pagination">');
				...
				if(strcmp($f_pp,'tout')!=0){
						...
						if($nb_pages>0){
							...
							if($page>=2){
								$precedent = $page-1;
										echo('<div class="disabled"><a href="javascript:products_draft_load_other_page(\'edit\',\''.$precedent.'\')">< Prec.</a></div>'); 
							}else{
							...
							if($count_pagination >2){		
								echo('<a title="page num&eacute;ro 1" href="javascript:products_draft_load_other_page(\'edit\',\'1\')" class="pgnt_nmbr"> 1 </a><span style="float:left; padding: 0px 3px 0px 3px;">..</span>');
							}
							...
							while( ($count_pagination<=$nb_pages) && ($count_local<$count_local_stop) ){
								if($count_pagination==$page){
									echo('<div class="disabled">&nbsp;'.$count_pagination.'&nbsp;</div>');
										...
										echo('<a title="page num&eacute;ro '.$count_pagination.'" href="javascript:products_draft_load_other_page(\'edit\',\''.$count_pagination.'\')" class="pgnt_nmbr"> '.$count_pagination.' </a>');	
								}
								$count_pagination++;
								$count_local++;
							}
							...
							if(($count_pagination <=$nb_pages) ){ //pour ne pas afficher suivant si on est dans la derniere page	
									echo('<span style="float:left; padding: 0px 3px 0px 3px;">..</span><a title="page num&eacute;ro '.$nb_pages.'" href="javascript:products_draft_load_other_page(\'edit\',\''.$nb_pages.'\')" class="pgnt_nmbr"> '.$nb_pages.' </a>');
							}
							
							//echo('] ');
							if($page<$nb_pages ){//partir a la derniere page (30) si on a les resultats qui depasse 300 
								$suivant = $page +1;
									echo('<a title="page num&eacute;ro '.$suivant.'" href="javascript:products_draft_load_other_page(\'edit\',\''.$suivant.'\')" style="text-decoration:none;" class="pgnt_nmbr">&nbsp;Suiv.&nbsp;</a>');
							}
						}
			...
			echo('<div class="products table-responsive">');
				...
				while($content_get_products	= $db->fetchAssoc($res_get_products)){
					...
					$row_link	= EXTRANET_URL.'extranet-v3-products-draft-edit.html?id='.$content_get_products['product_id'];
					...
					echo('<tr class="rs '.$modulo_local_class.'">');
						...
						echo('<td class="valign cursor-default">');
							...
							echo('<a href="'.EXTRANET_URL.'extranet_v3_product_preview.html?id='.$content_get_products['product_id'].'&t=e" title="Voir" target="blank">');
								echo('<i class="fa fa-eye"></i>');
							echo('</a>&nbsp;');
							
							echo('<a href="extranet-v3-products-draft-edit.html?id='.$content_get_products['product_id'].'" title="Modifier"><i class="fa fa-pencil"></i></a>');
								echo('&nbsp;');
								
							echo('<a href="javascript:void(0);" onclick="product_delete_draft(\''.$content_get_products['product_id'].'\', \'list_draft_edit\');" title="Supprimer"><i class="fa fa-remove"></i></a>');
								echo('&nbsp;');
			...
			echo("Vous n'avez actuellement aucun brouillon de modification.");
