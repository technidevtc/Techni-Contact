<?php
	...
	$stats_products_id		= mysql_escape_string($_POST['stats_products_hidden_id']);
	$type					= mysql_escape_string($_POST['type']);
	
	//To add it in the title of graph only !
	$stats_product_name		= mysql_escape_string($_POST['stats_product_name']);
	...
	//For the excel title !
	$stats_dates_excel_title		= '';
	$stats_dates_excel_title_end	= '';
	
	
	if(!empty($_SESSION['extranet_user_id'])){
	
		if(strcmp($type,'simple')==0 || ($stats_interval_v1==$stats_interval_v2)){
			//Simple
			...
			require_once('extranet_v3_stats_load_products_detail_external_file.php');
			...
			if(!empty($stats_interval_v1) && !empty($stats_interval_v2)){
					...
					while($stats_timestamp_start_while<=$stats_timestamp_end_while){
						...
						//For excel export
						$stats_dates_excel_title_end	= $stats_get_number_days_of_a_month.'/'.date('m/Y',$stats_timestamp_start_while).' 23:59:59';;
						...
						$query_month	= "SELECT 
											...
											AND 
												sh.idProduct=".$stats_products_id."
											AND 
					...
					$stats_generated_header		= '<b>'.$stats_global_count.'</b> visites sur <b>'.$stats_product_name.'</b> entre <b>'.$stats_timestamp_start_header_show.'</b> et <b>'.$stats_timestamp_end_header_show.'</b> ';
					...
					$stats_dates_excel_title	= ' du 01/'.substr($stats_interval_v1,4,2).'/'.substr($stats_interval_v1,0,4).' au '.$stats_dates_excel_title_end;
	?>
				document.getElementById('stats_h_month').value	= "<?php echo ($stats_dates_excel_title); ?>"; 
	...
	<?php
		...
		}//end else if(strcmp($type,'simple')==0)
