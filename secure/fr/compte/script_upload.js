...
function fileSelected_produit(photo_type) {
	
   if(photo_type=='adress_picture_produit'){
    // get selected file element
    var oFile = document.getElementById('adress_picture_produit').files[0];
   ...
   var rFilter = /^(image\/jpg|image\/gif|image\/jpeg|image\/png)$/i;
    ...
    if(photo_type=='adress_picture_produit'){
       // get preview element
        var oImage = document.getElementById('preview_facade_produit');
   } 
    var oReader = new FileReader();
        oReader.onload = function(e){
        // e.target.result contains the DataURL which we will use as a source of the image
