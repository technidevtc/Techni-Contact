...
$db = DBHandle::get_instance();
	...
	$sql_lead  = "SELECT id, email,id_user,prenom,nom,origin 
				  FROM contacts 
				  WHERE DATE_ADD( FROM_UNIXTIME( create_time ) , INTERVAL 20 MINUTE ) >= NOW() 
				  AND idProduct !='' 
				  AND origin IN ('Téléphone entrant','Click to call')
				 ";
				  // AND origin IN ('T&eacute;l&eacute;phone entrant','Click to call')
		...
		while($data_lead = mysql_fetch_object($req_lead)){
			...
			if(empty($data_check_email->id)){
				...
				$sql_insert = "INSERT INTO `feedback_u_operators_note` (
								...
								VALUES (NULL ,  '1',  '".$data_lead->id_user."',  '".$data_lead->email."',  '".$data_lead->id."',  
