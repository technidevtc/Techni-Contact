...
define("CATALOG_FILE", WWW_PATH."media/auto/criteo/products-simple-db.xml");
...
$fields_list = array (
	...
	"producturl" => array("source_type" => "UC", "default" => "__PRODUCT_URL__", "filter_type" => "url,urlext=?campaignID=01"),
	"smallimage" => array("source_type" => "UC", "default" => "__PRODUCT_IMAGE_THUMB_SMALL_URL__", "filter_type" => "url"),
		...
		foreach ($fields_list as $field_name => $set) {
			...
			switch($set["source_type"]) {
				...
				case "UC" :
					switch($set["default"]) {
						...
						case "__PRODUCT_CATEGORY_1__" :			
						//$pdt[$field_name] = $families[$families[$ref["idFamily"]]["idParent"]]["idParent"]; 
						if($ref['category'] == '0'){
						$pdt[$field_name] = 'Annonceur';
						}
						if($ref['category'] == '1'){
						$pdt[$field_name] = 'Fournisseur';	
						}
						if($ref['category'] == '2'){
						$pdt[$field_name] = 'Annonceur non facturé';	
						}
						if($ref['category'] == '3'){
						$pdt[$field_name] = 'Prospect';	
						}
						if($ref['category'] == '4'){
						$pdt[$field_name] = 'Annonceur bloqué';	
						}
						if($ref['category'] == '5'){
						$pdt[$field_name] = 'Litige de paiement';	
						}
            ...
            case "__PRODUCT_URL__" :
							...
							foreach($filters as $filter) {
								$filterSpecs = explode('=', $filter, 2);
								switch($filterSpecs[0]) {
									case "url": break;
									case "urlext":
										$pdt[$field_name] .= $filterSpecs[1];
										break;
									default: break;
								}
							}
