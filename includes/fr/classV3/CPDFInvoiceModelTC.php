...
class PDFInvoiceModelTC extends FPDF {
  ...
  function writeCGV() {
    ...
    $this->Write(3,
"
LA SOCIETE MD2I, Société par actions simplifiée (SAS) au capital de 160.000,00 euros, immatriculée au Registre du Commerce et des Sociétés de NANTERRE sous le numéro B 392 772 497, sise 253, rue Gallieni, 92100 BOULOGNE BILLANCOURT.
...
La SAS MD2I est la société éditrice et exploitante du site www.techni-contact.com.

Ce site est une galerie marchande destinée aux professionnels. A ce titre, elle propose à la vente des matériels, fournitures, équipements dont les professionnels et collectivités ont besoin dans le cadre de leur activité.

Une partie du catalogue disponible sur le site est proposée par des PARTENAIRES ANNONCEURS. Les demandes de devis concernant ces produits leur sont directement adressées, la SAS MD2I ne servant que d’intermédiaire à la mise en relation de ces derniers.

En souscrivant aux présentes Conditions Générales de Vente, LE CLIENT PROFESSIONNEL déclare que l’achat qu’il effectue sur la PLATEFORME répond directement à un besoin professionnel et qu’il commande le produit ou service directement en sa qualité de professionnel ; reconnaissant ainsi qu’il ne saurait en aucun cas être considéré comme un consommateur ou un non professionnel au sens de la Loi française.
");
    ...
    $this->SetFont('');
    $this->Write(3,
"Toute commande effectuée sur le site www.techni-contact.com implique obligatoirement l'acceptation sans réserve, par LE CLIENT PROFESSIONNEL, des présentes Conditions Générales de Vente.
...
");
    ...
    $this->SetFont('');
    $this->Write(3,
" : Société proposant une offre de produit ou service sur la PLATEFORME. Le PARTENAIRE ANNONCEUR est seul et entièrement responsable de l’exécution de la commande passée par un CLIENT PROFESSIONNEL pour les PRODUITS PARTENAIRES.
");
    $this->SetFont('','B');
    $this->text($this->x-5, $this->y+2.5,"•");
...
");
    ...
    $this->SetFont('');
    $this->Write(3,
" : s’entend du Produit ou Service qui est proposé et mis en ligne par les partenaires annonceurs de la société MD2I.
");
    $this->SetFont('','B');
    $this->text($this->x-5, $this->y+2.5,"•");
...
");
    ...
    $this->Write(3,
"La SAS MD2I pourra modifier à tout moment les présentes CGV. LE CLIENT PROFESSIONNEL sera informé de la nature de ces modifications dès leur mise en ligne sur le site www.techni-contact.com. 

Les modifications entreront en vigueur (XX) mois/jours après leur mise en ligne sur le site. 
...
Les caractéristiques figurant sur les catalogues, prospectus, fiches techniques ou tout autre document sont données à titre indicatif et ne sauraient en aucun cas engagées la responsabilité de la SAS MD2I.

En cas de doute sur les caractéristiques d’un PRODUIT COMMANDE, le Client doit contacter la SAS MD2I au 0.811.06.07.07 (coût d’un appel local non surtaxé).
...
");
    ...
    $this->Write(3,
"Les annonces des PRODUITS PARTENAIRES sont réalisées par les partenaires annonceurs de la SAS MD2I qui leur met à disposition un espace publicitaire. La SAS MD2I intervient uniquement dans la mise en relation du CLIENT PROFESSIONNEL et des partenaires annonceurs. Elle ne concourt à aucun moment dans la relation contractuelle qui peut s’établir entre le CLIENT PROFESSIONNEL et le partenaire annonceur.
");
...
Les prix des PRODUITS COMMANDES et le prix de livraison ne sont valables qu'en France métropolitaine.

Les frais engendrés par une livraison à l’étranger sont à la charge du CLIENT PROFESSIONNEL (impôts, taxes, redevances, frais de contrôle technique, frais éventuellement dus à l’application d’une législation étrangère…).
");
    $this->AddPage();
...
");
    ...
    $this->text($this->x-5, $this->y+2,"-");
    $this->Write(3,
"un jour ouvré pour le paiement par carte bancaire (visa/mastercard),
");
    $this->text($this->x-5, $this->y+2,"-");
    $this->Write(3,
"2 jours ouvrés pour le virement bancaire et le paiement via un compte PAYPAL, 
");
    $this->text($this->x-5, $this->y+2,"-");
    $this->Write(3,
"ainsi que de 3 à 4 jours ouvrés à compter de la réception du chèque bancaire ou du mandat administratif.
");
    $this->SetLeftMargin(20);
    $this->Ln();
    $this->Write(3,
...
");
    ...
    $this->Write(3,
"Les PRODUITS COMMANDES sur la Boutique en Ligne peuvent être livrés en France Métropolitaine ou à l’étranger sous certaines conditions visées supra. 

Les LIVRAISONS s'effectuent du lundi au vendredi. La SAS MD2I s’engage à respecter les délais de LIVRAISON indiquée pour les PRODUITS COMMANDES en vente directe sur le site www.techni-contact.com, excepté les mois de Juillet et d’Août ou ces derniers pourront être rallongés sans préavis. 
...
");
    ...
    $this->Write(3,
"La SAS MD2I propose sur son site www.techni-contact.com des produits partenaires. En effet, la SAS MD2I met à disposition de ses Partenaires-annonceurs des espaces publicitaires privés où ces derniers peuvent mettre en ligne leur produit.

La société MD2I n’interfère à aucun moment dans la vente du PRODUIT PARTENAIRE, elle joue le rôle d’intermédiation entre le CLIENT PROFESSIONNEL et le Partenaire-annonceur.

Dans ces conditions, elle ne saurait voir engager sa responsabilité en cas de problème résultant de cette relation contractuelle entre le CLIENT PROFESSIONNEL et le Partenaire-annonceur.
");

    // article 11
    $this->writeCGVtitle("GARANTIES");
    $this->Write(3,
"Le délai de garantie contre les vices-cachés est d’un an, à compter de la date de facturation et sous condition que le vice en cause ne figure pas dans la liste ci-dessous :
");
    $this->SetLeftMargin(35);
    $this->Ln();
    $this->text($this->x-5, $this->y+2,"-");
    $this->Write(3,
"lorsque le vice provient de la matière fournie par le client, d’une conception qui a imposée, ou des conditions de stockage inadaptées,
");
    $this->text($this->x-5, $this->y+2,"-");
    $this->Write(3,
"les incidents survenus dans tous les cas de force majeure ou fortuite, événements de guerre ou troubles intérieurs,
");
    $this->text($this->x-5, $this->y+2,"-");
    $this->Write(3,
"lorsque le vice provient de la simple négligence du CLIENT PROFESSIONNEL, de l’usure normale des fournitures, du défaut d’entretien du PRODUIT COMMANDE ou encore de l’utilisation ou de la mise en route de celui-ci par le CLIENT PROFESSIONNE non conformément aux instructions de la SAS MD2I ou suivant les règles de l’art de la profession,
");
    $this->text($this->x-5, $this->y+2,"-");
    $this->Write(3,
"lorsque des défectuosités proviennent d’une surcharge du matériel ou lorsqu’une pièce ou une partie d’appareil défectueuse entraîne des dégâts sur l’ensemble de l’appareil ou équipements environnants livrés par la SAS MD2I
");
    $this->SetLeftMargin(20);
    $this->Ln();
    $this->Write(3,
"LE CLIENT PROFESSIONNEL devra justifier de la matérialité de ces vices par écrit et dans le délai d’un an à compter de la facturation sous lettre recommandée avec avis de réception adressée à la SAS MD2I.
La SAS MD2I procédera aux mesures nécessaires afin de remédier à ce vice-caché, c’est-à-dire, la remise en état ou le remplacement de toutes pièces reconnues défectueuses.
");

    // article 12
    $this->writeCGVtitle("PROPRIETE INTELLECTUELLE");
    $this->Write(3,
"Les marques, logos, slogans, graphismes, photographies, animations, vidéos et textes contenus sur le site www.techni-contact.com sont la propriété intellectuelle exclusive de la SAS MD2I ou de ses partenaires et ne peuvent être reproduits, utilisés ou représentés sans l'autorisation expresse de la SAS MD2I ou de ses partenaires, sous peine de poursuites judiciaires. 
...
Chaque CLIENT PROFESSIONNEL s'interdit notamment de modifier, copier, reproduire, télécharger, diffuser, transmettre, exploiter commercialement et/ou distribuer de quelque façon que ce soit les services, les pages du site www.techni-contact.com, ou les codes informatiques des éléments composant les services et le site www.techni-contact.com.
");    
...
Toutes les données à caractère personnel recueillies par la SAS MD2I via le formulaire « ESPACE CLIENT » disponible sur le site www.techni-contact.com sont obligatoires pour devenir le CLIENT PROFESSIONNEL et sont nécessaires pour bénéficier de l’ensemble des Produits et Services proposés par la SAS MD2I. Ces données collectées directement auprès du CLIENT PROFESSIONNEL sont traitées par la SAS MD2I uniquement pour permettre la mise en œuvre et la gestion des Services proposés par la SAS MD2I et pour gérer les comptes personnels des CLIENTS PROFESSIONNELS.

La SAS MD2I déclare respecter scrupuleusement les législations françaises et européennes en matière de protection des données à caractère personnel et le site www.techni-contact.com a été déclaré auprès de la Commission Nationale de l'Informatique et des Libertés (CNIL) sous le numéro XXXX.
...
Les CLIENTS PROFESSIONNELS reconnaissent en outre être avisés de l'implantation d'un \"cookie\" dans leur ordinateur, destiné à enregistrer des informations relatives à la navigation sur le site www.techni-contact.com (informations fournies, pages consultées, date et heure de la consultation, etc.), en vue de faciliter la navigation sur le site en mémorisant certains paramètres. 
Conformément aux articles 38 et suivants de la loi n° 78-17 du 6 janvier 1978 modifiée relative à l'informatique, aux fichiers et aux libertés, toute personne peut obtenir communication et, le cas échéant, rectification ou suppression des informations la concernant, en s'adressant au service clientèle : contact@appart-maison.fr. Il est rappelé que toute personne peut, pour des motifs légitimes, s'opposer au traitement des données la concernant.
");    
...
");
    ...
    $this->Write(3,
"Les présentes CGV constituent l’intégralité de l’accord conclu entre l’ABONNE et la SAS MD2I.
